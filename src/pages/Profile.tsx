    ...
    <div className="mx-auto max-w-md px-4 pb-28 pt-4 safe-top">
        ...
        {editing ? (
          <div className="w-full max-w-xs">
            <input
              ...
              className="mb-2 w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-center text-sm outline-none"
            ...
            <textarea
              ...
              rows={2}
              className="w-full resize-none rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-center text-sm outline-none"
              placeholder="Bio"
            />
          </div>
        ) : (
          <>
            ...
            <p className="max-w-xs text-sm text-white/60">{user.bio || 'No bio yet.'}</p>
          </>
        )}
      </header>
      ...
      <div className="mb-6 grid grid-cols-3 gap-2">
        <div className="rounded-2xl border border-base-border bg-base-card p-3 text-center">
          <p className="text-lg font-extrabold">{displayedBalance !== undefined ? formatHype(displayedBalance) : '—'}</p>
          ...
          {editing ? (
            <>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="tap-scale flex-1 rounded-full border border-white/15 py-3 text-sm font-semibold"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  disabled={busy}
                  className="tap-scale flex-1 rounded-full bg-white py-3 text-sm font-bold text-black disabled:opacity-40"
                >
                  {busy ? 'Saving…' : 'Save'}
                </button>
              </div>
            ...
            <>
              <button
                ...
                className="tap-scale w-full rounded-full border border-white/15 py-3 text-sm font-semibold"
              >
                View full portfolio
              </button>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => logout()}
                  className="tap-scale flex-1 rounded-full border border-white/15 py-3 text-sm font-semibold text-accent-down"
                >
                  Log out
                </button>
              </div>
